...
function buildLog(level: string, message: string, metadata: any = {}) {
  return {
    level,
    message,
    metadata,
...
const processTask = async (task: Task) => {
  ...
  try {
    switch (task.type) {
      case "MONITOR_LOGS":
        ...
        await LogService.process(
          buildLog("info", `Monitored logs for ${task.payload.source}`, {
      ...
      case "TRIGGER_WORKFLOW":
        ...
        await createNotionPage(
          ...
          `Workflow Triggered: ${task.payload.workflowName}`,
    ...
    await LogService.process(
      buildLog("info", `Task ${task.type} completed`, { taskId: task.id })
    );
    ...
    await LogService.process(
      buildLog("error", `Task ${task.type} failed`, {
        taskId: task.id,
        error: error.message,
      })
    );
