...
exports.costTracker = {
  recordGeminiUsage: function (model, promptTokens, completionTokens, jobType) {
    ...
    logger_1.logger.info(
      'Gemini Usage Recorded: Model='
        .concat(model, ', Tokens=')
        .concat(totalTokens, ', Cost=$')
        .concat(costEstimate.toFixed(5), ', JobType=')
        .concat(jobType || 'N/A'),
    );
  ...
  recordGeminiAnalytics: function (analytics) {
    ...
    logger_1.logger.info(
      'Gemini Analytics Recorded: Model='
        .concat(analytics.model, ', JobType=')
        .concat(analytics.jobType || 'N/A', ', Duration=')
        .concat(analytics.durationMs || 'N/A', 'ms'),
    );
  ...
  getTotalCostEstimate: function () {
    return usageRecords.reduce(function (sum, record) {
      return sum + record.costEstimate;
    }, 0);
  },
